...
export function NewNoteCard({onNoteCreate}:NewNoteCardProps) {
  const [shoulShowOnboard, setShouldShowOnboard] = useState(true);
  ...
  function handleStartRecording(){
    ...
    speechRecognition.onresult = (event) =>{
     const transcription = Array.from(event.results).reduce((text, result) => text.concat(result[0].transcript), '')
